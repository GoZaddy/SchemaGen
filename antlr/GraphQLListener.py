...
class GraphQLListener(ParseTreeListener):
    ...
    def enterTypeDefinition(self, ctx: GraphQLParser.TypeDefinitionContext):
        print(ctx)
        print('type def found!')
        child = ctx.children[0]  # type: GraphQLParser.TypeDefinitionContext
        print(child.name().getText()) #prints name of GraphQL type
        print(child.fieldsDefinition().fieldDefinition(0).getText()) # get first field for this GraphQL type
